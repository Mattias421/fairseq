...
# Zsh specific ${lg:l} replaced with tr command
ph_lg=$(echo "$lg" | tr '[:upper:]' '[:lower:]')
...
ESPEAK_PATH=''
if [ "$phonemizer" = 'espeak' ]; then
  ESPEAK_PATH=$(which espeak)
elif [ "$phonemizer" = 'espeak-ng' ]; then
  ESPEAK_PATH=$(which espeak-ng)
elif [ "$phonemizer" = 'G2P' ]; then
  ESPEAK_PATH=''
else
  echo "Unknown phonemizer $phonemizer. Valid options are espeak, espeak-ng and G2P"
  exit 1
fi
...
if [ -z "$ESPEAK_PATH" ]; then
  ...
  echo "one is ${one}"
  # Bash sed syntax for inline editing is standard on Linux (GNU sed), but on macOS requires an empty string argument (-i '')
